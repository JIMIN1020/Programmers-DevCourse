...
exports.getBookDetail = `SELECT *, (SELECT EXISTS (SELECT * FROM likes WHERE user_id = ? AND liked_bookId = book.id)) AS liked, (SELECT COUNT(*) FROM likes WHERE liked_bookId = book.id) AS likes FROM book LEFT JOIN category ON book.category_id = category.category_id WHERE book.id = ?`;
